...
public class EOConditions {
	...
	public static final ConditionFactory<LivingEntity> PIGLIN_SAFE = createEntityCondition(new ConditionFactory<>(new Identifier(ConsularsOrigins.MODID, "piglin_safe"), new SerializableData(), (instance, entity) -> entity.world.getDimension().isPiglinSafe()));
	
	public static final ConditionFactory<Pair<DamageSource, Float>> CROSSBOW_ARROW = createDamageCondition(new ConditionFactory<>(new Identifier(ConsularsOrigins.MODID, "crossbow_arrow"), new SerializableData(), (instance, damageSourceFloatPair) -> damageSourceFloatPair.getLeft().getSource() instanceof PersistentProjectileEntity projectile && projectile.isShotFromCrossbow()));
